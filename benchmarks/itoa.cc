...
template <typename MissRate /* in percentage */, typename BenchSize, typename CacheSize>
struct BenchOptions;

template <size_t MissRate /* in percentage */, size_t BenchSize, size_t CacheSize>
struct BenchOptions<IntWrapper<MissRate>, IntWrapper<BenchSize>, IntWrapper<CacheSize>> {
  ...
  static constexpr size_t MAX_VALUE = CACHE_SIZE + (CACHE_SIZE * MISS_RATIO / (1 - MISS_RATIO));
};

template <typename BenchSize, typename CacheSize>
using LowMissRateBenchOptions = BenchOptions<IntWrapper<5>, BenchSize, CacheSize>;

template <typename BenchSize, typename CacheSize>
using HighMissRateBenchOptions = BenchOptions<IntWrapper<50>, BenchSize, CacheSize>;

TEMPLATE_PRODUCT_TEST_CASE("Benchmark itoa", "[benchmarks]", (LowMissRateBenchOptions, HighMissRateBenchOptions), ((Iterations<1'000>, CacheSize<100>), (Iterations<100'000>, CacheSize<10'000>))) {
  ...
  BENCHMARK("lamerman/lrucache") {
    auto cache = make_lamerman_lru_cache<int, std::string, CACHE_SIZE>(ToString);
    size_t total_size = 0;
    for (int key : keys) {
      total_size += cache[key].size();
    }
  ...
  BENCHMARK("vpetrigo/caches") {
    auto cache = make_vpetrigo_lru_cache<int, std::string, CACHE_SIZE>(ToString);
    size_t total_size = 0;
    for (int key : keys) {
      total_size += cache[key].size();
    }
  ...
  BENCHMARK("goldsborough/caches") {
    auto cache = make_goldsborough_lru_cache<int, std::string, CACHE_SIZE>(ToString);
    size_t total_size = 0;
    for (int key : keys) {
      total_size += cache[key].size();
    }
